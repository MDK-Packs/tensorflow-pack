...
def run_debug( device_name, debug_file_name ):
  ...
  if fvp_executable_name:
    result = subprocess.run( [ fvp_executable_name, "--config-file", os.path.join( test_on_arm_base, "templates", ( fvp_executable_name + ".config" ) ), debug_file_name], capture_output = True, text = True )
...
def build_cproj( toolchain, project_file_name ):
  ...
  result = subprocess.run( [ cbuild_script_name, project_file_name ], capture_output = True, text = True )
  if result.returncode:
    ...
    debug_file_name = os.path.join( os.path.join( os.path.dirname( project_file_name ) , "" ), os.path.basename( project_file_name ).rstrip( ".cprj" ) )
    if "GCC" == toolchain:
...
def create_cproj( cvariant, toolchain, device_name, test_file_name ):
  test_directory_name = os.path.join( test_on_arm_base, "work", cvariant, toolchain, device_name )
  template_directory_name = os.path.join( test_on_arm_base, "templates", cvariant, toolchain, device_name )

  template_file_name = os.path.join( template_directory_name, device_name ) + ".cprj"
  template_file = open( template_file_name, "rt" )
  if template_file:
    test_executable_name = os.path.basename( test_file_name ).rstrip( ".cc" )
    cproj_file_name = os.path.join( test_directory_name, test_executable_name ) + ".cprj"
    cproj_file = open( cproj_file_name, "wt" )
    if cproj_file:
      for input_line in template_file:
        if 0 < input_line.find( "_executable_to_test" ):
          output_line = input_line.replace( '_executable_to_test', test_executable_name )
        elif 0 < input_line.find( "_file_to_test" ):
          output_line = input_line.replace( '_file_to_test', os.path.abspath( test_file_name ) )
        else:
          output_line = input_line
        cproj_file.write( output_line )
      cproj_file.close( )
      try:
        shutil.copytree(os.path.join(template_directory_name,"RTE"), os.path.join(test_directory_name,"RTE"))
      except:
        e = sys.exc_info()[0]
...
def run_test_on_arm( cvariant, toolchain, device_name, search_path, log_file, junit_report ):
  total_counter = 0
  success_counter = 0
  failure_counter = 0
  global last_stderr
  ...
  for file_name in os.listdir( os.path.join(os.getcwd(), search_path) ):
    if file_name.endswith( "_test.cc" ):
      ...
      if device_name:
        print( ">>>TEST Running test unit: "+ file_name + " on device=" + device_name + " toolchain=" + toolchain + " cvariant=" + cvariant)
        startTime = perf_counter()
        cproj_file_name = create_cproj( cvariant, toolchain, device_name, test_file_name )
        ...
        if cproj_file_name:
          ...
          if debug_file_name:
            ...
            tc = TestCase(classname=file_name, name=tcname, elapsed_sec=(perf_counter() - startTime))
            if 0 == Error:
              tc = TestCase(classname=file_name, name=tcname, elapsed_sec=(perf_counter() - startTime), stderr=last_stderr, stdout=last_stdout)
              ...
              tc = TestCase(classname=file_name, name=tcname, elapsed_sec=(perf_counter() - startTime), stderr=last_stderr, stdout=last_stdout)
              print( ">>!TEST failure: tests failed " + test_file_name )
              log_file.write( ", %s" % "failure: debug result" )
              tc.add_failure_info("Unit Tests report failed Tests")
          ...
          tc = TestCase(classname=file_name, name=tcname, elapsed_sec=(perf_counter() - startTime), stderr=last_stderr, stdout=last_stdout)
          print( ">>!TEST failure: project generation failed" + test_file_name )
          log_file.write( ", %s" % "failure: cprj file" )
          tc.add_failure_info("Test project creation failed")
          failure_counter = failure_counter + 1
          junit_report.append(tc)
      else:
        log_file.write( ", %s" % test_file_name[ test_file_name.find( "tensorflow/lite/micro" ):len( test_file_name )] )
...
def main( ):
  ...
  parser = argparse.ArgumentParser( description = "Run tensorflow-lite-micro tests on Arm VFPs." )
  # add the --tflm_path option
  parser.add_argument( "--tflm_path", type = str, default = os.path.curdir, help = "path, where to look for the *_test.cc files" )
  parser.add_argument( "--toolchain", type = str, default = "ARMCLANG", help = "toolchain to build the tests with (ARMCLANG, GCC)" )
  parser.add_argument( "--cvariant", type = str, default = "CMSIS-NN", help = "variant of tensorflow lite micro to use (Reference, CMSIS-NN, Ethos-U)" )
  # parse the arguments
  ...
  log_file = open( os.path.join( test_on_arm_base, "test_on_arm_results_" ) + datetime_string + ".csv", "w" )
  ...
  result = run_test_on_arm( 0, 0, 0, tflm_path, log_file, 0)
  ...
  if result[ 0 ]:
    ...
    for test_device in test_devices:
      ...
      print( "testing device:", test_device )
      work_path_name = os.path.join(  test_on_arm_base, "work", cvariant ,toolchain , test_device )
      ...
      log_file.write( "\n%s" % test_device )
      result = run_test_on_arm( cvariant, toolchain, test_device, tflm_path, log_file, junit_cases)
      print( "total:", result[ 0 ], "success:", result[ 1 ], "failure:", result[ 2 ] )
      if result[ 2 ]:
        # something went wrong
        test_error = 1 
  else:
    print( "Error: no files to test in \"", tflm_path, "\", use --tflm_path option!" )
    test_error = 1 
  print (cvariant, toolchain)
  properties = {"cvariant" : cvariant, "toolchain" : toolchain}
  print (junit_cases)
  junit_report = TestSuite("Tensorflow Tests", junit_cases, package=test_folder, properties=properties)
  print(TestSuite.to_xml_string([junit_report]))
